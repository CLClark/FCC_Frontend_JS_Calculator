...
class CalcApp extends React.Component {
  ...
  operAddOld(opName) {
    return new Promise((resolve, reject) => {
      let currStack = Array.from(this.state.opStack);
      let lastOp; //this is the first operation value

      if (currStack.length > 0) {
        lastOp = currStack[currStack.length - 1]; //set the operation object
  ...
  operAdd(opName) {
    const transOp = this.transpileOpCode(opName); //alternate logic:
  ...
  procEquals() {
    let currEval = Array.from(this.state.currentEval); //alternate logic:

    let regex = /[\+\-\\\*\=]/g;
    let lastString = new String(currEval[currEval.length - 1]);

    if (lastString.match(regex)) {
      //final item is an operator... remove it and calculate
    ...
    this.operAdd("equals").then(finalStack => {
      ...
      let processedStack = this.procStack(finalStack); //return value as firstValue in new stack, object of length 1
  ...
  switchLastOperator(whatOp) {
    const opString = this.transpileOpCode(whatOp); //replaces the last object in the eval array with a new operator
