...
class CalcApp extends React.Component {
      ...
      if (currEval.length > 0) {
        ...
        if (lastString.match(regex) && this.state.newValue) {
          ...
          return this.setState({
            ...
            resolve(Array.from(this.state.currentEval)); // return (Array.from(this.state.currentEval));	
          });
        } else {
          //operator did not match, push number and operator
          currEval.push(this.state.currentNum, transOp); // currEval.push(transOp);
  ...
  procEquals() {
    ...
    let currEval = Array.from(finalStack); //alternate logic:
    ...
    if (lastString.match(regex)) {
      ...
      console.log("result is:" + opResult); // document.getElementById("display").innerText = opResult;
  ...
  render() {
    return React.createElement("div", {
      ...
      id: "display",
      style: disValues
    }, this.state.currentNum)), React.createElement(Button, {
      styleIn: buttonStyle,
